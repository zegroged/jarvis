"""yazilim_kaynak.json -> Workflow YAZILIM damitma scripti (.js).

Guvenlik hattinin (damit_ileri_uret.py) yazilim karsiligi: yazilim-pratisyen prompt
(mimari/algoritma/dil-inceligi/sistem-tasarimi/best-practice). alan="yazilim".
Idempotent, LF-only.

Kullanim: python uretim_araclari/damit_yazilim_uret.py [PARTI]
Sonra Workflow(scriptPath=uretim_araclari/wf_damit_yazilim.js).
"""
import json
import sys
import unicodedata
...
KAYNAK = KOK / "uretim_araclari" / "yazilim_kaynak.json"
QA_DIR = KOK / "data" / "processed" / "instruct_tr"
JS_HEDEF = KOK / "uretim_araclari" / "wf_damit_yazilim.js"
...
def main():
    ...
    if not parti:
        print("Damitilacak yok.")
        JS_HEDEF.write_text(
            "export const meta={name:'damit-yazilim',description:'bos',phases:[{title:'Damit'}]}\nlog('eksik yok')\nreturn {dokuman:0}\n",
